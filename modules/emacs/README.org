      ...
      (smie-prec2->grammar
       (smie-bnf->prec2
        '((exp
           ...
           ("{" exp "}")
           ("[" exp "]"))))))
(defun nushell-smie-rules (kind token)
  (pcase (cons kind token)
    ...
    (`(:list-intro . "") 0)))
(defun nushell-mode-hook ()
  (smie-setup nushell-smie-grammar 'nushell-smie-rules))
(add-hook 'nushell-mode-hook #'nushell-mode-hook)
#+end_src
...
(defun work-p (msg)
  (string-prefix-p "/supabase/" (mu4e-message-field msg :maildir)))
#+end_src
...
(use-package mu4e
  ...
  (add-to-list 'mu4e-bookmarks
               '(:name "Inboxes"
                 :query "m:/personal/Inbox OR m:/supabase/Inbox OR m:/university/Inbox"
                 :key ?i))
